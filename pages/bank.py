...
def read_docx_paragraphs(source):
    """
    Hàm đọc paragraphs từ 
file .docx với cơ chế tìm kiếm đa dạng:
    ...
    try:
        ...
        try:
             # Cơ 
chế 2: Thư mục làm việc hiện tại
             doc = Document(source)
        except Exception:
            try:
...
"""
    ...
    # FIX 
2: Regex cho prefix đáp án cần loại bỏ. Phải có dấu chấm/ngoặc HOẶC ít nhất một khoảng trắng sau chữ cái.
 ...
 clean_p = clean_text(p)
        ...
        # --- NEW QUESTION LOGIC 
(Fixing the Q40/3.5 issue) ---
        must_switch_q = (
            is_first_line or                             # Case 1: Start of doc
            is_q_start_phrased or                        # Case 
2: Explicit phrase
...
# --- APPLY SWITCH DECISION ---
  ...
  q_text = clean_p
       ...
       current["question"] = q_text
            ...
            if is_question_started 
and not is_max_options_reached:
                is_correct = False
...
{q["question"]}</div>', unsafe_allow_html=True)
        ...
        for opt in q["options"]:
            ...
            if clean_text(opt) == clean_text(q["answer"]):
                # Đáp án đúng: Xanh lá
                color_style = "color:#00ff00; text-shadow: 
...
def display_test_mode(questions, bank_name, key_prefix="test"):
    ...
    if f"{test_key_prefix}_submitted" not 
in st.session_state:
  ...
  st.rerun()
    ...
    if not st.session_state[f"{test_key_prefix}_submitted"]:
        ...
        q_key = f"{test_key_prefix}_q_{i}_{hash(q['question'])}" 
            ...
            st.markdown('<div class="question-separator"></div>', unsafe_allow_html=True) 
        if st.button("✅ 
Nộp bài Test", key=f"{test_key_prefix}_submit_btn"):
...
min-height: 100vh !important;
    ...
    padding: 0 !important;
    overflow: auto;
/* Giữ nguyên overflow: auto để có thanh cuộn */
    position: relative;
}}
...
BANK_OPTIONS = 
["----", "Ngân hàng Kỹ thuật", "Ngân hàng Luật VAECO", "Ngân hàng Docwise"]
...
st.session_state.current_group_idx = 0
    ...
    if not questions:
        st.error(f"❌ Không đọc được câu hỏi nào từ file 
**{source}**. Vui lòng kiểm tra file và cấu trúc thư mục (đảm bảo file nằm trong thư mục gốc hoặc thư mục 'pages/').")
        st.stop() 
...
start, end = idx * group_size, min((idx+1) * group_size, total)
            ...
            with col_all_bank:
                if st.button("📖 Hiển thị toàn bộ 
ngân hàng", key="btn_show_all"):
...
{q["question"]}</div>', unsafe_allow_html=True)
                        # Đảm bảo radio button có giá trị mặc định để tránh lỗi
                        default_val = st.session_state.get(q_key, q["options"][0] if q["options"] else None)
                        st.radio("", q["options"], index=q["options"].index(default_val) if default_val in q["options"] else 
0, key=q_key)
                        st.markdown('<div class="question-separator"></div>', unsafe_allow_html=True)
...
này", key="reset_group"):
   ...
   st.session_state.pop(f"q_{i}_{hash(q['question'])}", None) 
        ...
        if st.session_state.current_group_idx < len(groups) - 1:
            ...
            else: st.warning("Không có câu hỏi trong nhóm 
này.")
