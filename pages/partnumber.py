...
with col1:
    zone = st.selectbox("📂 Zone", sheet_options)
zone_selected = zone != CHOOSE_PROMPT
...
if zone_selected and ac_exists:
    aircraft_options = [CHOOSE_PROMPT] + sorted(df_base["A/C"].unique().tolist())
    with col2:
        aircraft = st.selectbox("✈️ Loại máy bay", aircraft_options)
    if aircraft != CHOOSE_PROMPT:
        df_filtered = df_filtered[df_filtered["A/C"] == aircraft]
else:
    aircraft = CHOOSE_PROMPT

if "DESCRIPTION" in df_filtered.columns:
    desc_options = [CHOOSE_PROMPT] + sorted(df_filtered["DESCRIPTION"].unique().tolist())
    with col3:
        desc = st.selectbox("🔑 Mô tả chi tiết", desc_options)
    if desc != CHOOSE_PROMPT:
        df_filtered = df_filtered[df_filtered["DESCRIPTION"] == desc]

if "ITEM" in df_filtered.columns:
    item_options = [CHOOSE_PROMPT] + sorted(df_filtered["ITEM"].unique().tolist())
    with col4:
        item = st.selectbox("🔌 Item", item_options)
    if item != CHOOSE_PROMPT:
...
if len(df_filtered) > 0:
    st.markdown('<h3 style="text-align:center; color:#FFEA00; text-shadow:0 0 15px #FFEA00;">KẾT QUẢ TRA CỨU</h3>', unsafe_allow_html=True)
    ...
    st.info("💡 Vui lòng chọn đủ thông tin để hiển thị kết quả.")
