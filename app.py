...
if "is_mobile" not in st.session_state:
    ...
    if ua_string:
        ...
        # Sử dụng st.stop() để chờ kết quả xác định thiết bị
        st.stop()
...
def intro_screen(is_mobile=False):
    ...
    bg_file = BG_MOBILE if is_mobile else BG_PC # LẤY FILE ẢNH NỀN TĨNH
    ...
    try:
        with open(bg_file, "rb") as bg_f: # ĐỌC FILE ẢNH NỀN TĨNH
            bg_b64 = base64.b64encode(bg_f.read()).decode() # MÃ HÓA ẢNH NỀN
    except FileNotFoundError:
        st.error(f"Không tìm thấy file hình nền: {bg_file}. Vui lòng kiểm tra lại đường dẫn.")
        st.stop()
    ...
    <head>
        ...
        .shattered-piece {{
            background-image: url("data:image/jpeg;base64,{bg_b64}"); /* DÙNG ẢNH NỀN TĨNH */
            background-repeat: no-repeat;
            background-size: cover; /* Đảm bảo hình nền bao phủ các mảnh vỡ */
            opacity: 0;
    ...
    <body>
        ...
        for (let i = 0; i < COLS * ROWS; i++) {{
            ...
            // Tính toán vị trí nền để mỗi mảnh hiển thị đúng phần của ảnh nền
            // Background size được đặt là 'cover' ở CSS, nên background-position tính theo % của container
            const x_pos_of_bg = (col / (COLS - 1)) * 100;
            const y_pos_of_bg = (row / (ROWS - 1)) * 100;
            
            piece.style.cssText = `
                background-position: ${{x_pos_of_bg}}% ${{y_pos_of_bg}}%; /* SỬ DỤNG VỊ TRÍ ẢNH NỀN */
                width: ${{size_x}}%;
                height: ${{size_y}}%;
                position: absolute;
                top: ${{row * size_y}}%;
        ...
        function activateShatter() {{
            ...
            // Kích hoạt hiển thị và hiệu ứng tan vỡ
        ...
        function finishIntro() {{
            ...
            setTimeout(() => {{
                // Kích hoạt reload Streamlit thông qua query param
