...
# THỜI GIAN CHUYỂN TRANG (tính bằng milliseconds)
# Đảm bảo thời gian này dài hơn thời lượng video thực tế
INTRO_DURATION_MS = 8500 

# THỜI GIAN CHỜ SAU KHI RERUN (tính bằng giây - ĐỘ TRỄ QUAN TRỌNG)
# Giúp Streamlit có thời gian tải và dựng trang mới trước khi intro bị gỡ bỏ.
POST_TRANSITION_DELAY_S = 0.3 
...
# ========== MÀN HÌNH INTRO (ĐÃ ĐƠN GIẢN HÓA LOGIC CHUYỂN TRANG) ==========
def intro_screen(is_mobile=False):
    ...
    try:
        ...
        # Đọc file background để PRELOAD
    ...
    <head>
        ...
        <style>
        /* CSS giữ nguyên, thêm preload-bg */
        html, body {{ margin: 0; padding: 0; overflow: hidden; background: black; height: 100%; }}
        video {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; }}
        #static-frame {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; background-size: cover; opacity: 0; z-index: 20; transition: opacity 0.1s linear; }}
        audio {{ display: none; }}
        #intro-text {{
            position: absolute; top: 8%; left: 50%; transform: translate(-50%, 0); width: 90vw; text-align: center; color: #f8f4e3;
            ...
            text-shadow: 0 0 15px rgba(255,255,230,0.4); animation: lightSweep 6s linear infinite, fadeInOut 6s ease-in-out forwards;
        ...
        #black-fade {{ position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: black; z-index: 100; transition: opacity 1s ease-in-out; opacity: 1; }}
        /* THẺ PRELOAD BACKGROUND (THAY ĐỔI BASE64 TẠI ĐÂY) */
        #preload-bg {{
    ...
    <body>
        ...
        function finishIntro() {{
            ...
            // Dùng cờ 'intro_done' để Python biết cần chuyển trang
        ...
        vid.addEventListener('canplay', startMedia);
...
# =========================================================================
# ========== LUỒNG CHÍNH (ĐÃ THÊM THỜI GIAN CHỜ) ==========
# =========================================================================
...
if not st.session_state.intro_done:
    ...
    if st.query_params.get('intro_completed') == '1':
        ...
        # === BƯỚC KHẮC PHỤC MẠNH MẼ: THÊM ĐỘ TRỄ ===
        # Dùng time.sleep() để ép Streamlit chờ. Điều này nghe có vẻ phản trực giác,
        # nhưng nó cho phép Streamlit xử lý tất cả các thay đổi DOM và tải tài nguyên
        # của trang chính trong khi màn hình intro cũ vẫn đang hiển thị.
        
        if 'transition_delay_done' not in st.session_state:
            st.session_state.transition_delay_done = False
            
        if not st.session_state.transition_delay_done:
            time.sleep(POST_TRANSITION_DELAY_S)
            st.session_state.transition_delay_done = True
            st.rerun() # Rerender lần nữa để áp dụng state

        # Sau khi delay, chuyển cờ và chuyển trang
        st.session_state.intro_done = True
        
        if 'intro_completed' in st.query_params:
    ...
    # Nếu không phải trạng thái chuyển, hiển thị màn hình intro
        ...
        # Reset cờ delay cho lần chạy tiếp theo
        st.session_state.transition_delay_done = False
