...
# (Phần hide_streamlit_ui giữ nguyên)
...
def intro_screen(is_mobile=False):
    ...
    <body>
        ...
        function finishIntro() {{
            ...
            // QUAN TRỌNG: Gắn tham số truy vấn và chuyển hướng
            const currentUrl = window.parent.location.href.split('#')[0]; // Lấy URL không hash
            const separator = currentUrl.includes('?') ? '&' : '?';
            const newUrl = currentUrl.split('?')[0] + separator + 'intro_played=true';
            window.parent.location.href = newUrl;
        }}

        // Logic play video/audio (giữ nguyên)
...
# ========== LUỒNG CHÍNH ĐÃ CHỈNH SỬA VỚI RESET URL ==========
...
if not st.session_state.intro_done:
    # --- TRẠNG THÁI A: ĐANG CHỜ PHÁT VIDEO HOẶC PHÁT HIỆN VIDEO KẾT THÚC ---
    ...
    if query_params.get("intro_played") == ["true"]:
        ...
        # QUAN TRỌNG: Yêu cầu trình duyệt chuyển hướng ngay lập tức đến URL sạch
        st.markdown(
            f"""
            <script>
            // Lấy URL cơ sở (không tham số)
            const cleanUrl = window.parent.location.href.split('?')[0];
            // Chuyển hướng ngay lập tức để Streamlit nhận lại request mới
    ...
    # --- TRẠNG THÁI B: ĐÃ CHUYỂN TRẠNG THÁI THÀNH CÔNG ---
