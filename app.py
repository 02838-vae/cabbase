...
def get_base64_encoded_file(file_path):
    ...
    try:
        ...
        # Nếu đang chạy trên Streamlit Cloud, việc đọc file có thể thất bại.
        # Chúng ta chỉ báo lỗi và stop.
...
try:
    ...
    # Giả định các file này tồn tại trong thư mục gốc.
...
# --- PHẦN 2: CSS CHÍNH (FIX PLAYER) ---

# Đổi key cũ "music_player" thành "player_widget" để tránh xung đột
PLAYER_KEY = "player_widget" 
...
/* ... (CSS cho Nền và Tiêu đề giữ nguyên) ... */

/* ======================================================= */
/* === FIX DỨT ĐIỂM CHO PLAYER IFRAME WRAPPER (Dùng data-testid mới) === */
/* ======================================================= */
...
/* Wrapper Streamlit của thành phần HTML nhúng Player */
[data-testid="stHtml-{PLAYER_KEY}"] {{
    opacity: 0 !important; 
...
.video-finished [data-testid="stHtml-{PLAYER_KEY}"] {{
    opacity: 1 !important; 
...
<script>
    function sendBackToStreamlit() {{
        ...
        try {{
            // Sử dụng key mới để tìm iframe
            const playerIframe = window.parent.document.querySelector('[data-testid="stHtml-{PLAYER_KEY}"] iframe');
            if (playerIframe) {{
...
# =======================================================
#               MUSIC PLAYER TÙY CHỈNH (Sử dụng key mới)
# =======================================================
...
# SỬ DỤNG KEY MỚI
st.components.v1.html(custom_music_player_html, height=80, key=PLAYER_KEY)
