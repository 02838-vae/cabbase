...
def intro_screen(is_mobile=False):
    ...
    <body>
        ...
        function finishIntro() {{
            ...
            // 🔥 THAY ĐỔI DỨT ĐIỂM: Gắn tham số truy vấn và chuyển hướng
...
if not st.session_state.intro_done:
    ...
    if query_params.get("intro_done_flag") == ["true"]:
        ...
        # Mục đích là xóa tham số truy vấn và kích hoạt lần tải lại cuối cùng
        st.markdown(
            """
            <script>
            ...
            // Ghi đè lịch sử trình duyệt để tránh back lại trang có param
            window.parent.location.replace(url.toString());
            </script>
            """, 
            unsafe_allow_html=True
        )
        # Dừng luồng Streamlit để JS xử lý chuyển hướng (rerun)
        st.stop() 
