...
# Thêm CSS cho Player tại đây để nó không bị nhúng dưới dạng văn bản
...
/* iframe cho Music Player */
/* Sử dụng [data-testid="stHtml"] để nhắm mục tiêu vào iframe của player */
/* CHỈNH SỬA QUAN TRỌNG: Đặt Player IFRAME CỐ ĐỊNH */
[data-testid="stHtml"] + div > iframe:last-child {{
    ...
    height: 100px !important; /* Chiều cao cố định */
    opacity: 0; /* Ban đầu ẩn */
    transition: opacity 1s ease-in-out;
}}

.video-finished [data-testid="stHtml"] + div > iframe:last-child {{
...
/* CSS NỘI BỘ CHO PLAYER (DÙNG TRONG IFRAME) */
...
# -----------------------------------------------------------
# --- PHẦN 4: MUSIC PLAYER CỐ ĐỊNH (SỬ DỤNG IFRAME RIÊNG) ---
# -----------------------------------------------------------
    ...
    <script>
        ...
        progressContainer.addEventListener('click', (e) => {{
            ...
            if (!isNaN(duration)) {{
                ...
                playSong(); // Phát ngay sau khi tua
            }}
        }});
        ...
        // Cố gắng tự động phát khi nhận được sự kiện video-finished từ parent (Đã được CSS bên ngoài xử lý)
        // Tuy nhiên, việc phát tự động trong iframe bị giới hạn, cần click.
        // Logic playSong() sau khi video kết thúc đã được chuyển sang hàm phụ.
        
        // Thêm một listener cho body để kích hoạt phát lần đầu
        document.body.addEventListener('click', () => {{
...
# NHÚNG MUSIC PLAYER BẰNG st.components.v1.html (IFRAME RIÊNG)
# Height và width được CSS bên trên ghi đè để đặt Fixed
st.components.v1.html(music_player_iframe_content, height=100, width=200, scrolling=False)
