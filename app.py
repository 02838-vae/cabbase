...
# ========== MÀN HÌNH INTRO (ĐÃ THÊM PRELOAD BACKGROUND) ==========
def intro_screen(is_mobile=False):
    hide_streamlit_ui()
    video_file = VIDEO_MOBILE if is_mobile else VIDEO_PC
    
    # Lấy file background TƯƠNG ỨNG của trang chính
    ...
    <head>
        ...
        /* THÊM PRELOAD: Tải background ảnh của trang chính ngay trong intro (ẩn đi) */
        #preload-bg {{
...
def main_page(is_mobile=False):
    ...
    try:
        # Lại đọc file để dùng base64 trong CSS
    ...
    html, body, .stApp {{
        ...
        /* Giảm animation fadeInBg nếu cần để tăng tốc độ cảm nhận */
...
if not st.session_state.intro_done:
    ...
    # === NHẬN THÔNG ĐIỆP TỪ JS (Giữ nguyên logic này) ===
    components.html("""
    <script>
    window.addEventListener("message", (event) => {
        if (event.data.type === "intro_completed_rerun") {
