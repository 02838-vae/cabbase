...
# File ảnh nền của trang chính (sẽ hiện ra sau khi màn đen tan)
BG_PC = "cabbase.jpg"
...
SHATTER_FADE_OUT_DELAY = 0.5 # Thời gian chờ sau khi tan vỡ trước khi màn hình tối đen
MAIN_PAGE_FADE_DURATION = 1.5 # Thời gian trang chính hiện ra (Fade In)
...
# ========== MÀN HÌNH INTRO (ĐÃ BỎ RECONSTRUCT) ==========
def intro_screen(is_mobile=False):
    ...
    js_shatter_fade_out_delay = SHATTER_FADE_OUT_DELAY * 1000
    js_main_page_fade_duration = MAIN_PAGE_FADE_DURATION * 1000
    ...
    <head>
        ...
        /* Đặt ảnh nền trang chính (BG) vào thân body của iframe để nó lộ ra khi màn đen tan */
        body {{
    ...
    <body>
        ...
        const SHATTER_FADE_OUT_DELAY = {js_shatter_fade_out_delay}; 
        const MAIN_PAGE_FADE_DURATION = {js_main_page_fade_duration}; 
        ...
        function finishIntro() {{
            ...
            setTimeout(() => {{
                ...
                blackFade.style.transition = 'opacity 0.2s linear'; 
            }}, SHATTER_DURATION + SHATTER_FADE_OUT_DELAY); 

            // BƯỚC 3: Hiệu ứng cinematic cho trang chính (Fade In)
            setTimeout(() => {{
                // Giảm độ mờ của lớp phủ đen để lộ trang chính nằm dưới
                blackFade.style.transition = 'opacity ' + (MAIN_PAGE_FADE_DURATION / 1000) + 's ease-in-out';
                blackFade.style.opacity = 0; 
                
                // BƯỚC 4: Thông báo hoàn thành - Tải lại trang (Sau khi fade xong)
                setTimeout(() => {{
                    window.parent.postMessage({{type: 'intro_done'}}, '*');
                }}, MAIN_PAGE_FADE_DURATION + 10); 

            }}, SHATTER_DURATION + SHATTER_FADE_OUT_DELAY + BLACKOUT_DELAY); 
...
# ========== TRANG CHÍNH (ĐÃ CHỈNH ANIMATION FADEIN) ==========
...
def main_page(is_mobile=False):
    ...
    # Điều chỉnh thời gian fadeInBg để khớp với MAIN_PAGE_FADE_DURATION (1.5s)
    st.markdown(f"""
    <style>
    html, body, .stApp {{
        height: 100vh !important;
        ...
        animation: fadeInBg {MAIN_PAGE_FADE_DURATION}s ease-in-out forwards; 
    }}
    .stApp::after {{
        content: "";
        position: absolute;
    ...
    .welcome {{
        ...
        animation: textLight 10s linear infinite, fadeIn 1.5s ease-in-out forwards; 
        letter-spacing: 2px;
        z-index: 3;
    }}
    @keyframes textLight {{
