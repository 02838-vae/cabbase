...
# ========== HÀM CALLBACK CHO NÚT ẨN ==========
def finish_intro_callback():
    """Được gọi khi nút ẩn được click bởi JavaScript để chuyển trạng thái."""
...
if "is_mobile" not in st.session_state:
    ...
    if ua_string:
        ...
        # Nếu chưa lấy được User Agent, tạm dừng.
...
def intro_screen(is_mobile=False):
    ...
    <body>
        ...
        function finishIntro() {{
            ...
            // 2. Gửi tín hiệu sang Streamlit để click nút ẩn
            setTimeout(() => {{
...
def main_page(is_mobile=False):
    ...
    st.markdown(f"""
<style>
/* Đảm bảo căn lề sát bên trái để tránh lỗi cú pháp */
html, body, .stApp {{
    height: 100vh !important;
    background: 
...
/* Ẩn nút kích hoạt của Streamlit (Khắc phục lỗi kẹt) */
/* Selector này nhắm vào container của nút st.button(type="secondary") */
...
if not st.session_state.intro_done:
    ...
    # 1. Khởi tạo và ẩn nút kích hoạt
    btn_placeholder = st.empty()
    # Thêm type="secondary" để nút có data-testid="stFormSubmitButton"
...
window.addEventListener("message", (event) => {
    if (event.data.type === "intro_done_internal") {
        // Tìm nút ẩn và kích hoạt sự kiện click để buộc Streamlit Rerun
...
""", unsafe_allow_html=True)
    ...
    # 4. Cơ chế Dự phòng Python Timer (8.6s > 8.5s tổng thời gian)
    time.sleep(8.6)
    if not st.session_state.intro_done:
        st.session_state.intro_done = True
        st.rerun()
