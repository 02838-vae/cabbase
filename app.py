...
AUDIO_PLANE = "media/plane_fly.mp3"
MUSIC_FILES = [
...
if st.session_state.is_mobile is None:
    st.session_state.intro_done = False
    ua_string = st.runtime.scriptrunner.get_script_run_ctx().request.headers.get("User-Agent", "")
    if ua_string:
...
def intro_screen(is_mobile=False):
    ...
    if not os.path.exists(video_path):
        ...
        st.rerun()
        return
    if not os.path.exists(AUDIO_PLANE):
        st.error(f"⚠️ Không tìm thấy audio: {AUDIO_PLANE}")
        st.session_state.intro_done = True
        st.rerun()
        return
    ...
    with open(AUDIO_PLANE,"rb") as f:
        audio_b64 = base64.b64encode(f.read()).decode()

    # Vị trí video & chữ
    object_position = "center bottom" if is_mobile else "center"
    text_bottom = "30%" if is_mobile else "18%"
    ...
    vid.oncanplay = function(){{
        vid.play().catch(function(e){{console.log("Autoplay blocked")}})
